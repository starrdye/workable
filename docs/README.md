...
| `0.40-personal` | **Workflow Group Hierarchy**: Recursive node counting in sidebar, parent-subgroup relationship editor in left panel, cycle protection for nesting. **UI Polishing**: Click-based sidebar dropdowns for stability, centered group labels at all zoom levels, minimalist text-only labeling, **group-level filtering (dimming)**. **Logic Fixes**: Removed node duplication in auto-nesting via "Move" behavior. |
| `0.40.2-personal` | **Production Fixes**: Fixed `isDemoMode` TypeScript build error in `StartScreen` and `AISettingsModal`. Gated the "Demo Environment" floating badge in `page.tsx` behind an `isDemoMode` flag. Updated `i18n.ts` with missing demo-related keys. |
